...
export const PostinganPage: React.FC = () => {
  ...
  const [activeView, setActiveView] = useState<'buat' | 'minggu_ini' | 'arsip'>('buat');
  ...
  useEffect(() => {
    ...
    return () => unsubscribe();
  }, [effectiveTelegramId, hasUserEditedStartNumber]);

  // Sync displayed posts based on activeView & selectedDay
  useEffect(() => {
    if (activeView === 'minggu_ini') {
      if (selectedDay === 'Semua') {
        setPosts(allCurrentWeekPosts);
      } else {
            ...
            {posts.length === 0 && !isLoadingHistory ? (
              <div className="py-20 text-center bg-slate-900/40 rounded-3xl border border-slate-800/50">
                ...
                <p className="text-xs font-bold text-slate-500">
                  {activeView === 'minggu_ini' 
                    ? (selectedDay === 'Semua' ? 'Belum ada postingan minggu ini' : `Belum ada postingan untuk hari ${selectedDay}`)
                    : 'Folder arsip kosong'}
                </p>
              </div>
            ) : (
